...
final class SettingsWindowController: NSWindowController {
    ...
    private func header() -> NSView {
        ...
        NSLayoutConstraint.activate([
            ...
            brand.trailingAnchor.constraint(equalTo: view.trailingAnchor),
            brand.centerYAnchor.constraint(equalTo: view.centerYAnchor)
        ])
        return view
    }
    ...
    private func statusCard() -> NSView {
        ...
        statusTitle.textColor = Brand.text
        statusTitle.translatesAutoresizingMaskIntoConstraints = false
        statusDetail.font = .systemFont(ofSize: 12)
        statusDetail.textColor = Brand.secondary
        ...
        NSLayoutConstraint.activate([
            statusDot.leadingAnchor.constraint(equalTo: card.leadingAnchor, constant: 20),
            statusDot.topAnchor.constraint(equalTo: card.topAnchor, constant: 22),
            statusDot.widthAnchor.constraint(equalToConstant: 10),
            statusDot.heightAnchor.constraint(equalToConstant: 10),
            statusTitle.leadingAnchor.constraint(equalTo: statusDot.trailingAnchor, constant: 10),
            statusTitle.centerYAnchor.constraint(equalTo: statusDot.centerYAnchor),
            ...
            permissionButton.trailingAnchor.constraint(equalTo: card.trailingAnchor, constant: -20),
            permissionButton.centerYAnchor.constraint(equalTo: card.centerYAnchor)
        ])
        return card
    }
    ...
    private func controlsCard() -> NSView {
        ...
        rows.addArrangedSubview(toggleRow("WinMax enabled", "Pause window overrides without quitting.", enabledSwitch, #selector(toggleEnabled)))
    ...
    private func menuBarCard() -> NSView {
        let card = CardView()
        card.heightAnchor.constraint(equalToConstant: 92).isActive = true
        let heading = section("MENU BAR")
        let row = toggleRow(
            "Menu Vault",
            "Search and open Accessibility-exposed status items. Shortcut: ⌃⌥⌘V.",
            menuVaultSwitch,
            #selector(toggleMenuVault)
        )
        let open = NSButton(title: "Open Vault", target: self, action: #selector(openMenuVault))
        ...
        NSLayoutConstraint.activate([
            ...
            row.trailingAnchor.constraint(equalTo: open.leadingAnchor, constant: -16),
            row.topAnchor.constraint(equalTo: heading.bottomAnchor, constant: 9),
            open.trailingAnchor.constraint(equalTo: card.trailingAnchor, constant: -20),
    ...
    private func startupCard() -> NSView {
        ...
        let row = toggleRow(
            "Launch WinMax at login",
            "Start automatically after signing in to macOS.",
            loginSwitch,
    ...
    private func diagnosticsCard() -> NSView {
        ...
        let detail = label(
            "Local debug logs only. No telemetry, window titles or Menu Vault labels are logged.",
            11,
            .regular,
            Brand.secondary
        )
        ...
        NSLayoutConstraint.activate([
            ...
            logs.trailingAnchor.constraint(equalTo: card.trailingAnchor, constant: -18),
            logs.centerYAnchor.constraint(equalTo: card.centerYAnchor),
            copy.trailingAnchor.constraint(equalTo: logs.leadingAnchor, constant: -8),
            copy.centerYAnchor.constraint(equalTo: logs.centerYAnchor)
        ])
        return card
    }
    ...
    private func toggleRow(
        ...
        row.heightAnchor.constraint(equalToConstant: 33).isActive = true
        let titleLabel = label(title, 13, .medium, Brand.text)
        let detailLabel = label(detail, 10.5, .regular, Brand.secondary)
        detailLabel.lineBreakMode = .byTruncatingTail
        toggle.target = self
        toggle.action = action
        toggle.translatesAutoresizingMaskIntoConstraints = false

        [titleLabel, detailLabel, toggle].forEach(row.addSubview)
        NSLayoutConstraint.activate([
            ...
            detailLabel.trailingAnchor.constraint(lessThanOrEqualTo: toggle.leadingAnchor, constant: -12),
    ...
    private func section(_ text: String) -> NSTextField {
        label(text, 10, .bold, Brand.accent)
    }

    @objc private func refresh() {
        let trusted = AXIsProcessTrusted()
        if !trusted {
            statusDot.layer?.backgroundColor = Brand.warning.cgColor
            statusTitle.stringValue = "Accessibility permission required"
            statusDetail.stringValue = "Window control and Menu Vault need Accessibility access."
            permissionButton.title = "Grant Access"
        } else if !WindowController.shared.isEventTapInstalled {
            statusDot.layer?.backgroundColor = Brand.warning.cgColor
            statusTitle.stringValue = "Starting window controller…"
            statusDetail.stringValue = "Permission is granted. WinMax is preparing its event interceptor."
            permissionButton.title = "Check Again"
        } else if !settings.enabled {
            statusDot.layer?.backgroundColor = Brand.warning.cgColor
            statusTitle.stringValue = "Window control is paused"
            statusDetail.stringValue = settings.menuVaultEnabled
                ? "Window overrides are paused. Menu Vault remains available."
                : "Window overrides are paused. Your settings are preserved."
            permissionButton.title = "Accessibility"
        } else {
            statusDot.layer?.backgroundColor = Brand.accent.cgColor
    ...
    @objc private func toggleGreen() {
        settings.overrideGreenButton = greenSwitch.state == .on
    }

    @objc private func toggleTitle() {
        settings.titleBarDoubleClick = titleSwitch.state == .on
    }

    @objc private func toggleShortcut() {
        settings.overrideFullscreenShortcut = shortcutSwitch.state == .on
    }
    ...
    @objc private func toggleMenuVault() {
        settings.menuVaultEnabled = menuVaultSwitch.state == .on
    }
    ...
    @objc private func toggleLogin() {
        do {
            try LaunchAtLoginManager.shared.setEnabled(loginSwitch.state == .on)
        } catch {
            loginSwitch.state = LaunchAtLoginManager.shared.isEnabled ? .on : .off
...
private final class CardView: NSView {
    override init(frame: NSRect) {
        ...
        wantsLayer = true
        layer?.backgroundColor = Brand.panel.cgColor
        layer?.borderColor = Brand.border.cgColor
        layer?.borderWidth = 1
